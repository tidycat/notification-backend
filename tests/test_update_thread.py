...
class TestUpdateThread(unittest.TestCase):
    ...
    def setUp(self):
        patcher1 = patch('notification_backend.notification_threads.dynamodb_update_item')  # NOQA
    ...
    def test_missing_type_member(self):
        ...
        self.assertEqual(
            ...
            "Invalid 'type' member, should be 'threads'"
        )
        self.assertEqual(self.mock_db_update.mock_calls, [])

    def test_invalid_type_member(self):
        self.lambda_event['payload']['data']['type'] = "faketype"
        t = NotificationThreads(self.lambda_event)
        with self.assertRaises(TypeError) as cm:
            t.process_thread_event("update_thread")
        result_json = json.loads(str(cm.exception))
    ...
    def test_invalid_thread_id(self):
        self.lambda_event['payload']['data']['id'] = "33s"
        t = NotificationThreads(self.lambda_event)
        with self.assertRaises(TypeError) as cm:
            t.process_thread_event("update_thread")
        result_json = json.loads(str(cm.exception))
