...
class NotificationTags(object):
    ...
    def update_tag(self):
        ...
        tag_id = patch_payload.get('id')
        if not tag_id:
            error_msg = "Payload missing 'id' member"
            logger.info(error_msg)
            return format_response(400, format_error_payload(400, error_msg))

        # If no color is specified, we have nothing to do here
        tag_color = patch_payload.get('attributes', {}).get('color')
        if not tag_color:
            payload = {
                "meta": {
                    "message": "No color specified, nothing to do here"
                }
            }
            return format_response(200, payload)
        ...
        try:
            dynamodb_update_item(
                ...
                "set tag_color = :val",
                {":val": tag_color},
                Attr("user_id").eq(self.userid) & Attr("tag_name").eq(tag_name) & Attr("tag_color").ne(tag_color)  # NOQA
            )
        except ClientError as e:
            if e.response['Error']['Code'] == "ConditionalCheckFailedException":  # NOQA
                error_msg = "Tag %s already up to date, nothing to do here" % tag_name  # NOQA
                logger.info(error_msg)
                return format_response(403,
                                       format_error_payload(403, error_msg))
            error_msg = "Error updating tag %s in the datastore" % tag_name
            logger.error("%s: %s" % (error_msg, str(e)))
            return format_response(500, format_error_payload(500, error_msg))
        except (Boto3Error, BotoCoreError) as e:
            error_msg = "Error updating tag %s in the datastore" % tag_name
        ...
        payload = {
            "meta": {
                "message": "Tag %s updated successfully" % tag_name
            }
        }
        return format_response(200, payload)
